...
GameScene::GameScene(AssetManager* asset_manager, TileMap* level) : BaseScene(asset_manager), _camera(sf::FloatRect(0.f, 0.f, 256.0f, 240.f)), _gui_view(sf::FloatRect(0.f, 0.f, 256.0f, 240.f)) {
	...
	_fps_text = sf::Text("0 fps", *asset_manager->GetFont("Roboto"), 16);
	...
	RegisterActionSystem(std::bind_front(&GameScene::InputSystem, this));
	RegisterFixedUpdateSystem(std::bind_front(&GameScene::AISystem, this));
	RegisterFixedUpdateSystem(std::bind_front(&GameScene::LifetimeSystem, this));
	RegisterFixedUpdateSystem(std::bind_front(&GameScene::GravitySystem, this));
	RegisterFixedUpdateSystem(std::bind_front(&GameScene::MovementSystem, this));
	RegisterFixedUpdateSystem(std::bind_front(&GameScene::DetectCollisionSystem, this));
	//RegisterFixedUpdateSystem(std::bind_front(&GameScene::ResolveCollisionSystem, this));
	RegisterFixedUpdateSystem(std::bind_front(&GameScene::DestructionSystem, this));
	RegisterFixedUpdateSystem(std::bind_front(&GameScene::PlayerDeathSystem, this));
	RegisterFixedUpdateSystem(std::bind_front(&GameScene::PlayerVictorySystem, this));
	RegisterFixedUpdateSystem(std::bind_front(&GameScene::SetPlayerAnimationSystem, this));
	RegisterFixedUpdateSystem(std::bind_front(&GameScene::AnimationSystem, this));

	RegisterRenderSystem(std::bind_front(&GameScene::Render, this));
	RegisterRenderGUISystem(std::bind_front(&GameScene::DrawGUI, this));
}
...
void GameScene::Load(GameManager* gm) {
	...
	_entity_manager.sort<Transform>([](const Transform& t1, const Transform& t2) {
		return t1.position.x < t2.position.x;
	});
	// Sort such that higher z indices are drawn on top.
	_entity_manager.sort<Sprite>([](const Sprite& t1, const Sprite& t2) {
		return t1.z_index < t2.z_index;
	});

	auto animation_name = _level->player.fall_animation;
	auto ani_info = _asset_manager->GetAnimation(animation_name);
	auto& tex = std::get<sf::Texture&>(ani_info);
	auto spconfig = std::get<SpriteAnimationConfig>(ani_info);

	auto mario = _entity_manager.entity();
	_entity_manager.add<Mortal>(mario, PLAYER_STARTING_HEALTH);
	_entity_manager.add<Movement>(mario, 0.0f, 0.0f);
	_entity_manager.add<Transform>(mario,
	...
	_entity_manager.add<AABB>(mario,
		sf::Vector2f(_level->player.aabb_width, _level->player.aabb_height),
		AABB::Material::Solid,
		1,
		ENTITY_HARDNESS,
		PLAYER_SMALL_PIERCE);
	_entity_manager.add<Sensors>(mario);
	_entity_manager.add<Sprite>(mario, tex, sf::FloatRect((float)spconfig.x, (float)spconfig.y, (float)spconfig.w, (float)spconfig.h), sf::Vector2f(0.5f, 0.5f), PLAYER_Z_INDEX);
	_entity_manager.add<Animation>(mario,
		animation_name,
		spconfig,
		true,
		"",
	...
	// left boundary
	auto world_aabbs = _entity_manager.entity();
	_entity_manager.add<Transform>(world_aabbs, -1.0f, world_half_h);
	_entity_manager.add<AABB>(world_aabbs, sf::Vector2f(2.0f, world_h), AABB::Material::Solid);
	// right boundary
	world_aabbs = _entity_manager.entity();
	_entity_manager.add<Transform>(world_aabbs, world_w + 1.0f, world_half_h);
	_entity_manager.add<AABB>(world_aabbs, sf::Vector2f(2.0f, world_h), AABB::Material::Solid);
	...
	for (int i = 0; i < _level->width; i++) {
		...
		world_aabbs = _entity_manager.entity();
		_entity_manager.add<Transform>(world_aabbs, x, world_h + 1.0f);
		_entity_manager.add<AABB>(world_aabbs, sf::Vector2f(w, 2.0f), AABB::Material::Permeable, 999, 999, 999);
	}
	...
	for (auto i : _level->tilemap) {
		if (i > 0) {
			...
			auto ani_info = _asset_manager->GetAnimation(t.animation_name);
			auto& tex = std::get<sf::Texture&>(ani_info);
			auto spconfig = std::get<SpriteAnimationConfig>(ani_info);

			auto entity = _entity_manager.entity();
			_entity_manager.add<Transform>(entity, x + tile_width_half, y + tile_height_half);
			if (t.aabb_width > 0 && t.aabb_height > 0) {
				_entity_manager.add<AABB>(entity, sf::Vector2f(t.aabb_width, t.aabb_height), AABB::Material::Solid, 0, BLOCK_HARDNESS, 0);
			}
			_entity_manager.add<Sprite>(entity, tex, sf::FloatRect((float)spconfig.x, (float)spconfig.y, (float)spconfig.w, (float)spconfig.h), sf::Vector2f(0.5f, 0.5f), BACKGROUND_Z_INDEX);
			_entity_manager.add<Animation>(entity,
				t.animation_name,
				spconfig,
				true,
				"",
...
void GameScene::SetPlayerAnimationSystem(GameManager* gm) {
	...
	if (change_animation != "" && ani->name != change_animation) {
		auto ani_info = _asset_manager->GetAnimation(change_animation);
		auto& tex = std::get<sf::Texture&>(ani_info);
		auto spconfig = std::get<SpriteAnimationConfig>(ani_info);
...
void GameScene::Render(GameManager* gm, sf::RenderWindow* window, int delta_ms) {
	const Transform* player_t = _entity_manager.get<Transform>(_player);
	if (player_t != nullptr) {
